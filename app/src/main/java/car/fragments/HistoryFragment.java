...
import car.adapter.files.HistoryRecycleAdapter;
import car.jet.riderapp.HistoryActivity;
import car.jet.riderapp.HistoryDetailActivity;
                            ...
                            for (int i = 0; i < arr_rides.length(); i++) {
                                ...
                                if (generalFunc.getJsonValue("eCancelled", obj_temp.toString()).equals("Yes")) {
                                    ...
                                    if (generalFunc.getJsonValue("iActive", obj_temp.toString()).equals("Canceled")) {
                                        ...
                                        if (generalFunc.getJsonValue("iActive", obj_temp.toString()) == null || generalFunc.getJsonValue("iActive", obj_temp.toString()).equals("")) {
                                            if (generalFunc.getJsonValue("eStatus", obj_temp.toString()).equalsIgnoreCase("Pending")) {
                                                map.put("iActive", generalFunc.retrieveLangLBl("", "LBL_PENDING"));

                                            } else if (generalFunc.getJsonValue("eStatus", obj_temp.toString()).equalsIgnoreCase("Cancel")) {
                                                map.put("iActive", generalFunc.retrieveLangLBl("", "LBL_CANCELED_TXT"));

                                            } else if (generalFunc.getJsonValue("eStatus", obj_temp.toString()).equalsIgnoreCase("Assign")) {
                                                map.put("iActive", generalFunc.retrieveLangLBl("", "LBL_ASSIGNED"));

                                            } else if (generalFunc.getJsonValue("eStatus", obj_temp.toString()).equalsIgnoreCase("Accepted")) {
                                                map.put("iActive", generalFunc.retrieveLangLBl("", "LBL_ACCEPTED"));
                                            } else if (generalFunc.getJsonValue("eStatus", obj_temp.toString()).equalsIgnoreCase("Declined")) {
                                                map.put("iActive", generalFunc.retrieveLangLBl("", "LBL_DECLINED"));
                                            } else if (generalFunc.getJsonValue("eStatus", obj_temp.toString()).equalsIgnoreCase("Failed")) {
                                                map.put("iActive", generalFunc.retrieveLangLBl("", "LBL_FAILED_TXT"));
                                            } else {
                                                map.put("iActive", generalFunc.getJsonValue("eStatus", obj_temp.toString()));
                                            }
                                        }
                                    }
                                }
                                ...
                                if (generalFunc.getJsonValue("eType", obj_temp.toString()).equals(Utils.CabGeneralType_UberX) &&
                                    ...
                                    map.put("SelectedCategory", generalFunc.getJsonValue("vVehicleType", obj_temp.toString()));
                                }
                                map.put("eAutoAssign", generalFunc.getJsonValue("eAutoAssign", obj_temp.toString()));
