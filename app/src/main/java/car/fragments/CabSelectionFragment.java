...
import car.adapter.files.CabTypeAdapter;
import car.jet.riderapp.FareBreakDownActivity;
import car.jet.riderapp.MainActivity;
import car.jet.riderapp.R;
import car.jet.riderapp.RentalDetailsActivity;
import car.drawRoute.DirectionsJSONParser;
...
public class CabSelectionFragment extends Fragment implements CabTypeAdapter.OnItemClickList {
    ...
    MTextView rentalPkg;
    ImageView rentPkgImage;
    MTextView rentalPkgDesc;
    TextView plus;
    TextView minus;
    TextView number;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rentPkgImage = (ImageView) view.findViewById(R.id.rentPkgImage);
        rentalPkgDesc = (MTextView) view.findViewById(R.id.rentalPkgDesc);
        seatsLL = (LinearLayout) view.findViewById(R.id.seatsLL);
        amount_tv = (TextView) view.findViewById(R.id.amount_tv);
        plus = view.findViewById(R.id.plus);
        minus = view.findViewById(R.id.minus);
        number = (TextView) view.findViewById(R.id.number);
        plus.setOnClickListener(new setOnClickList());
        minus.setOnClickListener(new setOnClickList());
            ...
            public void setResponse(String responseString) {
                ...
                if (responseString != null && !responseString.equals("")) {
                    ...
                    if (action.equals("1")) {
                        ...
                        isCardValidated = true;
                        Utils.printLog("setCardSelection", ":: checkPaymentCard");
                        setCardSelection();
    ...
    public void generateCarType() {
        try {
            if (amount_tv.getText().toString().equals("")) {
                amount_tv.setText(mainAct.cabTypesArrList.get(0).get("total_fare"));
                amount = Float.parseFloat(mainAct.cabTypesArrList.get(0).get("total_fare").split(" ")[1]);
                percentage = Integer.parseInt(mainAct.cabTypesArrList.get(0).get("additional_percentage"));
                totalamount = Float.parseFloat(mainAct.cabTypesArrList.get(0).get("total_fare").split(" ")[1]);
            }
        } catch (Exception ex) {
        }
        if (cabTypeList == null) {
...
//        showLoader();
        ...
        for (int i = 0; i < mainAct.cabTypesArrList.size(); i++) {
            ...
            String ispool = mainAct.cabTypesArrList.get(i).get("isPool");
            String additional_percentage = mainAct.cabTypesArrList.get(i).get("additional_percentage");
            Utils.printLog("currentCabGeneralType", "::" + currentCabGeneralType);
            if (!eType.equalsIgnoreCase(currentCabGeneralType)) {
                continue;
            }
            map.put("additional_percentage", additional_percentage);
            map.put("isPool", ispool);
...
//            if (eRental != null && !eRental.equalsIgnoreCase("") && eRental.equalsIgnoreCase("No")) {
        ...
        if (!mainAct.iscubejekRental) {
            if (rentalTypeList.size() > 0) {
                if (generalFunc.getJsonValue("APP_TYPE", userProfileJson).equalsIgnoreCase(Utils.CabGeneralType_Ride) || (RideDeliveryType.equalsIgnoreCase(Utils.CabGeneralType_Ride) && mainAct.iscubejekRental == false)) {
                    ...
                    Runnable r = new Runnable() {
                        ...
                        public void run() {
                            try {
//puneet
                                Utils.printLog("bottomMargin", ":: Cabsel");
                                mainAct.setPanelHeight(290);
    ...
    public void onItemClick(int position) {
        ...
        after_seats(position);
        mainAct.getAvailableDriverIds();
        if (cabTypeList.get(position).get("isPool").equals("1")) {
            mainAct.setIsPool("1");
            mainAct.initializeLoadCab();
            seatsLL.setVisibility(View.VISIBLE);
            number.setText("1");
            mainAct.setSeats("1");

            amount_tv.setText(cabTypeList.get(position).get("total_fare"));
            try {
                amount = Float.parseFloat(cabTypeList.get(position).get("total_fare").split(" ")[1]);
                percentage = Integer.parseInt(cabTypeList.get(position).get("additional_percentage"));
                totalamount = Float.parseFloat(cabTypeList.get(position).get("total_fare").split(" ")[1]);
            } catch (Exception ex) {
    ...
    @Override
    public void onItemNotClicked(int position) {
        amount_tv.setText(cabTypeList.get(position).get("total_fare"));
        try {
            amount = Float.parseFloat(cabTypeList.get(position).get("total_fare").split(" ")[1]);
            percentage = Integer.parseInt(cabTypeList.get(position).get("additional_percentage"));
    ...
    public void after_seats(int position) {
                    ...
                    if (isDataAvail == true) {
                        ...
                        for (int i = 0; i < vehicleTypesArr.length(); i++) {
                            ...
                            if (distance != null) {
                                ...
                                Utils.printLog("CarType", "::" + generalFunc.getJsonValue("eType", obj_temp.toString()) + "::" + mainAct.getCurrentCabGeneralType());
                                    ...
                                    if (cabTypeList != null) {
                                        for (int k = 0; k < cabTypeList.size(); k++) {
                                            ...
                                            if (/*map.get("vVehicleType").equalsIgnoreCase(generalFunc.getJsonValue("vVehicleType", obj_temp.toString()))
                                                ...
                                                additional_percentage = generalFunc.getJsonValue("additional_percentage", obj_temp.toString());
...
//                    dialogShowOnce = true;
    ...
    public void outstandingDialog(boolean isReqNow) {
        ...
        adjustTitleTxt.setText(generalFunc.retrieveLangLBl("Adjust in Your trip", "LBL_ADJUST_TRIP_TXT"));
...
//                    }, 200);
                ...
                int p = Integer.parseInt(number.getText().toString());
                if (p > 1) {
//                    plus.setVisibility(View.VISIBLE);
//                    minus.setVisibility(View.GONE);
                    number.setText(String.valueOf(--p));
                    amount = amount - ((totalamount * percentage) / 100);
                    amount_tv.setText("$" + String.valueOf(amount));
                    mainAct.setSeats(String.valueOf(p));
                    mainAct.initializeLoadCab();
                }
            } else if (i == R.id.plus) {
                int p = Integer.parseInt(number.getText().toString());
...
//                    if (amount==0){
                    try {
                        //amount=Float.parseFloat(cabTypeList.get(0).get("total_fare").split(" ")[1]);
                        amount = amount + ((totalamount * percentage) / 100);
                        amount_tv.setText("$" + String.valueOf(amount));
                    } catch (Exception ex) {
                    }
