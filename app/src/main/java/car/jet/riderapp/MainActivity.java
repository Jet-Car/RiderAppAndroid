...
import car.adapter.files.UberXOnlineDriverListAdapter;
import car.datepicker.files.SlideDateTimeListener;
import car.datepicker.files.SlideDateTimePicker;
import car.dialogs.NoInternetConnectionDialog;
import car.dialogs.RequestNearestCab;
import car.fragments.CabSelectionFragment;
import car.fragments.DriverAssignedHeaderFragment;
import car.fragments.DriverDetailFragment;
import car.fragments.MainHeaderFragment;
import car.fragments.PickUpLocSelectedFragment;
import car.fragments.RequestPickUpFragment;
import car.general.files.AddDrawer;
import car.general.files.ConfigPubNub;
import car.general.files.CreateAnimation;
import car.general.files.ExecuteWebServerUrl;
import car.general.files.GeneralFunctions;
import car.general.files.GetAddressFromLocation;
import car.general.files.GetLocationUpdates;
import car.general.files.HashMapComparator;
import car.general.files.InternetConnection;
import car.general.files.LoadAvailableCab;
import car.general.files.MapAnimator;
import car.general.files.MyApp;
import car.general.files.StartActProcess;
import car.general.files.UpdateFrequentTask;
...
import car.utils.AnimateMarker;
...
import car.view.slidinguppanel.SlidingUpPanelLayout;
...
import static car.utils.Utils.CabGeneralType_Deliver;
import static car.utils.Utils.CabGeneralType_UberX;
import static car.utils.Utils.generateNotification;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (app_type.equalsIgnoreCase(Utils.CabGeneralTypeRide_Delivery_UberX)) {
            if (isUfx) {
                ...
                Utils.printLog("selectedCabTypeId", "::check::" + selectedCabTypeId);
...
//        }
        ...
        if (!isFinishing()) {
            ...
            paramsval.bottomMargin = Utils.dipToPixels(getActContext(), 200);
            if (driverAssignedHeaderFrag != null) {
                userTripBtnImgView.setVisibility(View.VISIBLE);
            }
    ...
    public void onLocationUpdate(Location location) {
        ...
        //create issue in after add
//        if (cameraPosition != null && !isMovedToCurLoc) {
//            getMap().moveCamera(CameraUpdateFactory.newCameraPosition(cameraPosition));
//            isMovedToCurLoc = true;
//
//        }
...
//
    ...
    private void setBottomView(final Marker marker) {
        ...
        if (isPickUpLocationCorrect() == false || map.size() == 0) {
            ...
            milesTxt.setText(generalFunc.CalculationByLocationKm(SourceLat, SourceLong, DesLat, DesLong) + generalFunc.retrieveLangLBl("km away", "LBL_KM_AWAY"));
        ...
        new SlideDateTimePicker.Builder(getSupportFragmentManager())
                .setListener(new SlideDateTimeListener() {
                    ...
                    public void onDateTimeSet(Date date) {
                        ...
                        if (!cabSelectionFrag.handleRnetalView(Utils.convertDateToFormat(Utils.DateFormatewithTime, date))) {
   ...
   public void setSeats(String seats){
    ...
    private void handleRequest(Intent data) {
        ...
        try {
            ...
            cabRequestedJson.put("isPool",isPool);
            cabRequestedJson.put("requiredSeats",seats);
    ...
    public void openSurgeConfirmDialog(String responseString, final String selectedTime, Intent data) {
        ...
        if (cabSelectionFrag != null && cabTypeList != null && cabTypeList.size() > 0 && cabTypeList.get(cabSelectionFrag.selpos).get("total_fare") != null && !cabTypeList.get(cabSelectionFrag.selpos).get("total_fare").equals("")) {
...
//                            finish();
    ...
    public void sendRequestToDrivers(String driverIds, String cabRequestedJson) {
        ...
        requestCabData.put("isPool",isPool);
        requestCabData.put("requiredSeats",seats);
        ...
        exeWebServer.setDataResponseListener(new ExecuteWebServerUrl.SetDataResponse() {
            ...
            public void setResponse(String responseString) {
                ...
                if (responseString != null && !responseString.equals("")) {
                    ...
                    String action = generalFunc.getJsonValue(CommonUtilities.action_str, responseString);
                    if (action.equals("999")){
                        generalFunc.showGeneralMessageWithicon("", generalFunc.retrieveLangLBl(generalFunc.getJsonValue(CommonUtilities.message_str, responseString),"LBL_FOR_MINIMUM_DISTANCE"),"ddd","fdf",getResources().getDrawable(R.drawable.uber),getResources().getDrawable(R.drawable.lyft));
...
//                }
    ...
    public void stopReceivingPrivateMsg() {
        ...
        if (ConfigPubNub.getInstance(true) != null) {
            Utils.printLog("releaseInstances", "::" + "stopReceivingPrivateMsg");
    ...
    public void callBackEvent(boolean status) {
        try {
            ...
            if (cabSelectionFrag == null) {
                ...
                Utils.printLog("gMapClear", "Called");
...
//
    ...
    public void setNoLocViewEnableOrDisabled(boolean show) {
        ...
        if (generalFunc.getJsonValue("APP_TYPE", userProfileJson).equalsIgnoreCase(Utils.CabGeneralTypeRide_Delivery_UberX)) {
            nolocmenuImgView.setVisibility(View.GONE);
        }
...
//                            gMap.moveCamera(CameraUpdateFactory.newLatLngBounds(builder.build(), width - Utils.dipToPixels(getActContext(), 80), metrics.heightPixels - Utils.dpToPx(getActContext(), 300), 100));
//                            gMap.moveCamera(CameraUpdateFactory.scrollBy(0, Utils.dpToPx(getActContext(), -200)));
...
//                    }
//                    if (mainHeaderFrag != null) {
//                        mainHeaderFrag.setPickUpAddress(pickUpLocationAddress);
//                        mainHeaderFrag.isPickUpAddressStateChanged(pickUpLocation);
//
//                    }
